...
from colmena.task_server.el import EnsembleTaskServer
...
if __name__ == '__main__':
    ...
    sys_config = SystemConfig(name="local",ncpus=12,ngpus=0, cpus=list(range(12)), gpus=[])

    # Create the task server and task generator
    my_sim = Simulation(Path('./simulate'))
    doer = EnsembleTaskServer([my_sim],queues, system_config=sys_config)
